...
**V的工作台** 是一个基于 GitHub Pages 的静态个人平台，用于展示两类内容：
...
### 卡片样式（提示词）
- 标题：14px 字体，加粗
- 内容：13px，灰色文字，`max-height: 4.5em` 折叠（约 3-4 行预览）
- 内容超过 200 字符时显示"展开全文"按钮
- 点击"展开全文"不是原地展开，而是弹出独立窗口

**展开/收起实现细节：**
- 响应式设计：根据屏幕宽度自动选择展示方式
  - **桌面端（>600px）**：Overlay Modal（居中弹窗）
    - 固定定位，半透明遮罩背景
    - 居中面板，最大宽度 600px，最大高度 85vh
    - 内容可滚动，带关闭按钮和点击遮罩关闭
    - 缩放入场动画（`scale: 0.95 → 1`）
  - **移动端（≤600px）**：Bottom Sheet（底部弹出面板）
    - 固定定位，半透明遮罩背景
    - 从底部滑入，圆角顶部
    - 顶部有拖拽手柄，支持手势拖拽关闭
    - 拖拽距离超过 80px 自动关闭
- 两种方式都支持 ESC 键关闭、点击遮罩关闭、点击 × 关闭
- 弹窗打开时锁定 body 滚动
- 窗口 resize 时自动关闭当前弹窗
